...
for(i in 1:100){
  save_targets[i,]= modelABC(c(0.2,0.02),100000,0.1)
}

# obtaining the targets as the mean number of infected at the time points
save_targ = c(ceiling(mean(save_targets[,1])),ceiling(mean(save_targets[,2])))
...
ABC_rej$computime
plot(ABC_rej$param[, 1], ABC_rej$param[, 2], main="ABC_rejection, beta=0.2, gamma=0.02, tolerance = 1")
