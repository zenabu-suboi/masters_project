...
abc = dbGetQuery(db, 'select J.*, P.*, M.* from job J, par P, met M 
                 where J.serial = P.serial and J.serial = M.serial')
dbDisconnect(db)
...
for (col in par_cols) {
  ...
  cat(paste(colname, '\n'))
  beanplot( abc[,colname] ~ abc$post_bool*abc$smcSet, 
            what=c(1,1,1,0), col=list(c(grey(0.3), 1,1, grey(0)),
                                      c(grey(0.9), 1,1, grey(0.7))), 
            main='', ylab=colname, side='both', bw='nrd0')
}
dev.off()
